...
Etant donnée que les streamers en cours ne sont pas primordiaux comparé à la base de données permanente qui est celle des streamers enregistrés, nous avons préféré partir sur une liste pour une simplicité des résultats. Si l'application devait se relancer et qu'un streamer était déjà en live il y aurait une nouvelle notification mais qui ne serait pas dévastatrice. A travers nos différents essais nous avons trouvé un bug très complexe à reproduire, malgré l'utilisation de la liste comme un cache des streamers déjà prévenu, il peut lui arriver aléatoirement de laisser passer le streamer déjà prévenu, alors qu'il est bien dans le cache. Après plus long travail en débogage nous avons remarqué que ce phénomène arrive à l'arrêt d'un thread de la solution. On peut donc se demander si le thread fermé ne laisse pas une erreur passer, un problème de concurrence de donnée qui donne à deux thread différent le même résultat, créant ainsi ce problème de duplicata. Le comportement est assez rare dans le programme, nous continuons d'investiguer sur une solution plus durable pour éviter ce problème.
